...
#include *i C:\Dropbox\AHKs\thirdParty\GDIp.ahk
...
Gui, Add, Slider, x516 y457 w110 h30 vSpeedSlider Range1-200 tooltip invert, 100
...
;some brushes
   ...
   pPRedTHICK				:= Gdip_CreatePen(0xffFF0000, 3)
   ...
   pPmissileRed1		:= Gdip_CreatePen(0xffE01B56, 1)
   pPmissileRed2		:= Gdip_CreatePen(0xffE01B56, 2)
   pPmissileRed3		:= Gdip_CreatePen(0xffE01B56, 3)
   pPmissileGreen		:= Gdip_CreatePen(0xff388034, 2)
   pBDashed				:= Gdip_BrushCreateHatch(0xffFFFF00, 0xffFF3399, 29)
   pPAttack				:= Gdip_CreatePenFromBrush(pBDashed, 2)
   pPTrajectory			:= Gdip_CreatePen(0x500000FF, 1)
...
AA_Build(stats, Class, Direction = "A", destinationx=0, destinationy=0) ; example: AA_Build(1011100, 1, "L")
{ 														; 1move, 2build, 3scan, 4attack, 5gather, 6hp, 7missile
global
If (class = "M") ;missile
	{
	;msgbox, missile build sequence initiated. targeting x=%x% and y=%y%
	;msgbox % "current unit position is" . %currentunit%_posx . "," . %currentunit%_posy
	;msgbox, destinationx = %destinationx% destinationy = %destinationy%
	xdistance := destinationx - %currentunit%_posx
	ydistance := destinationy - %currentunit%_posy
	xtimer := Abs(xdistance)
	ytimer := Abs(ydistance)
	xtimer := xtimer/40
	ytimer := ytimer/40
	;msgbox, X distance = %xdistance%, number of 40px movement turns to reach it = %xtimer%. Y distance = %ydistance%, number of 40px movement turns to reach it = %ytimer%
	xvalue := ((xDistance = 0) ? () : (((xDistance < 0) ? ("L") : ("R"))))
	yvalue := ((yDistance = 0) ? () : (((yDistance < 0) ? ("U") : ("D"))))
	Dir := xvalue . yvalue
	Directioncoords(Dir)
	;msgbox, direction towards destination is %dir%
	StringGetPos, Position, currentunit, _
	StringLeft, player, currentunit, Position
	unitID += 1
	newunit = %player%_%class%_%unitID%
	ADJUSTX := %currentunit%_posx + xvalue
	adjusty := %currentunit%_posy + yvalue
;	msgbox, xvalue=%xvalue%, yvalue=%yvalue% - this added to current unit position.
	%newunit%_posx := adjustx
	%newunit%_posy := adjusty
	%newunit%_cooldown = 1
	%newunit%_destinationx := X
	%newunit%_destinationy := Y
	%newunit%_dir := Dir
	%newunit%_type := "M"
	%newunit%_move := 3
	%newunit%_xtimer := xtimer
	%newunit%_ytimer := ytimer
	allunitlist .= newunit . "|"
	;msgbox, build finished, check vars for new unit: "%newunit%"
	;listvars
	;msgbox, continuing to draw phase.
	;listvars
	;msgbox, missile built - press ok to draw.
	AA_DrawUnit(newunit)
	return
	}
...
Cost := stats1+stats2+stats3+stats4+stats5+stats6+stats7
If (Cost > 6)
	return
StringGetPos, Position, currentunit, _
StringLeft, player, currentunit, Position
...
AA_Move(Direction) ; See below for potential directions.
{
	...
	If (%currentunit%_cooldown > 0)
		return 3
	;msgbox, moving to direction %direction%
	Directioncoords(direction) 
	;msgbox, coords for direction %direction% are %xvalue% and %yvalue%. adding these to moving unit's current position
...
If (xattackdistance <= attackrange) AND (yattackdistance <= attackrange) ;check if target is in range
	...
	Gdip_SetSmoothingMode(G, 4)
	Gdip_DrawLine(G, pPATTACK, attackanimationx1,attackanimationy1,attackanimationx2,attackanimationy2)
	Loop 5
		{
		point2 := a_index+1
...
If (%unit%_type = "M") ;missile
	{
	;msgbox, draw missile initiated...
	Gdip_SetSmoothingMode(G, 4)
	TrajectoryX := %unit%_destinationX + 10
	TrajectoryY := %unit%_destinationY + 10
	TrajectoryX2 := %unit%_posX + 10
	TrajectoryY2 := %unit%_posY + 10
	Gdip_DrawLine(G, pPTrajectory, TrajectoryX2,TrajectoryY2,TrajectoryX,TrajectoryY)
	shortLine := vectorGetShortLine(Trajectoryx2, Trajectoryy2, Trajectoryx, trajectoryy, 15)
	StringSplit, array_item_, shortLine, `,
	;msgbox, trajectory complete. click ok to draw shaft
	;msgbox, coords of missile shaft coming from %trajectoryx2%,%trajectoryy2% and traveling towards %trajectoryx%,%trajectoryy% is start= %array_item_1%,%array_item_2% and finish = %array_item_3%,%array_item_4%.
	Gdip_DrawLine(G, pPmissilegreen, array_item_1, array_item_2, array_item_3, array_item_4)
	;msgbox, draw missile finished.
	BitBlt(hdc_WINDOW,0, 0, 502,502, hdc_main,0,0)
	return
	}
If (%drawplayer% = 1)
...
missilesize := %unit%_missile
If (missilesize > 0)
	{
	buildline1x := %unit%_posx
	buildline1y := %unit%_posy+8
	buildline2x := %unit%_posx+8
	buildline2y := %unit%_posy
	Gdip_DrawLine(G, pPmissileRed%missilesize%, buildline1x, buildline1y, buildline2x, buildline2y)
	}
moverate := %unit%_move
If (moverate > 0)
	{
...
Loop %cdloop%
	...
	If (%cdunit%_type = "M")
		{
		;msgbox, missile cooldown launched.
			If (%cdunit%_hp = 0)
				continue
			Checkhitx := abs(%cdunit%_posx - %cdunit%_destinationX)
			Checkhity := abs(%cdunit%_posy - %cdunit%_destinationy)
			If (Checkhitx <= 20 and Checkhity <= 20)
				{
				If AA_CheckOccupancy(%cdunit%_destinationX, %cdunit%_destinationy) = 1
					{
					%checkunit%_hp -= 5
					Font = Arial
					textx := %checkunit%_posx+5
					texty := %checkunit%_posy-10
					Options = x%textx% y%texty% 0xffFF0000 s9
					Gdip_TextToGraphics(G, "-5 hp", Options, Font)
					}
				If (%checkunit%_hp < 1)
					UnitDeath(checkunit)
				UnitDeath(cdunit)
			...
			cdTrajectoryX := %cdunit%_destinationX
			cdTrajectoryY := %cdunit%_destinationY
			cdTrajectoryX2 := %cdunit%_posX
			cdTrajectoryY2 := %cdunit%_posY
			shortLine := vectorGetShortLine(cdTrajectoryx2, cdTrajectoryy2, cdTrajectoryx, cdtrajectoryy, 40)
			StringSplit, cdarray_item_, shortLine, `,
			;msgbox, setting missile's new coords after traveling 40px. old coords = %cdtrajectoryx2%, %cdtrajectoryy2%. new coords = %cdarray_item_3%, %cdarray_item_4%.
			%cdunit%_posx := cdarray_item_3
			%cdunit%_posy := cdarray_item_4
			}
	;---------end missile section
...
xvalueg := ((xDistance = 0) ? () : (((xDistance < 0) ? ("L") : ("R"))))
yvalueg := ((yDistance = 0) ? () : (((yDistance < 0) ? ("U") : ("D"))))
Dir := xvalueg . yvalueg
return Dir
}
...
{
global
;msgbox, attempting to launch missile at coords %x%,%y%
If (%currentunit%_missile = 0) ;check if launching unit has missile function module
	return 0
If (%currentunit%_cooldown > 0) ;check cooldown
	return 3
xattackdistance := abs(X - %currentunit%_posx) 
yattackdistance := abs(Y - %currentunit%_posy)
If (xattackdistance < 100) AND (yattackdistance < 100) ;check if target is too close (within 5 space)
	return 4 ;target too close
attackrange := %currentunit%_missile*100
;msgbox, target outside minimum range. continuing launch on %x%,%y%
If (xattackdistance <= attackrange) AND (yattackdistance <= attackrange) ;check if target is in range
	{
	Font = Arial
	textx := %currentunit%_posx+5
	texty := %currentunit%_posy-10
	Options = x%textx% y%texty% 0xffFF0000 s9
	Gdip_TextToGraphics(G, "Missile launched!", Options, Font)
	distanceCD := (xattackdistance + yattackdistance)/20
	%currentunit%_cooldown += %distanceCD%
	;msgbox, launched. check for text. continuing to build phase.
	AA_Build(0000000, "M","A", X, Y)
	return 1
	}
Else
	{
	;msgbox, target out of range. returning 2.
	return 2 ;If target is out of range return 2
	}
}
...
{
global
;msgbox, finding coords for direction %direction2%
If (direction2 = "UR" or direction2 = "RU")
	{
	xvalue = 20
	yvalue = -20
	}
If (direction2 = "UL" or direction2 = "LU")
	{
	xvalue = -20
	yvalue = -20
	}
If (direction2 = "DR" or direction2 = "RD")
	{
	xvalue = 20
	yvalue = 20
	}
If (direction2 = "LD" or direction2 = "DL")
	{
	xvalue = -20
	yvalue = 20
	}
...
If (direction2 = "A")
	...
	;msgbox, coords for %direction2% = xvalue %xvalue% and yvalue %yvalue%
return
}
...
vectorGetShortLine(startX, startY, finishX, finishY, normval)
{
   ...
   distanceVector := vectorMultiply(norm, normval) ;normval = 15 for shaft, 40 for movement
