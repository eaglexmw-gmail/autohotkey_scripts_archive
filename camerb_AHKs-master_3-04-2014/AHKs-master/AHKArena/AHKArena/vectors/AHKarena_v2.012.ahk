...
If (%unit%_type = "M") ;missile
	...
	shortLine := vectorGetShortLine(Trajectoryx2, Trajectoryy2, Trajectoryx, trajectoryy)
...
;----missile section----
	If (%cdunit%_type = "M")
		...
		Loop 2
			...
			If (%cdunit%_hp = 0)
				continue
			Direction := %cdunit%_Dir
			Directioncoords(direction)
			movex := %currentunit%_posx + xvalue
			movey := %currentunit%_posy + yvalue
			If (movex = %cdunit%_destinationx and movey = %cdunit%_destinationy)
				{
				If AA_CheckOccupancy(movex, movey) = 1
					{
					%checkunit%_hp -= 5
					Font = Arial
