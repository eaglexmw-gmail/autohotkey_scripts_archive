...
;some brushes
   ...
   pBORANGE				:= Gdip_BrushCreateSolid(0xffFF6633)
   pBORANGE55			:= Gdip_BrushCreateSolid(0x55FF6633)
...
Loop %gamecount%
	...
	Loop
		...
		Guicontrol, Text, maintext, MainText`nGame = %gamenumber% / %gamecount%`nTurn = %a_index%`nAvg. # Turns: %turnaverage%
		AA_LoadUnits()
...
If effectlist
	{
	StringSplit, runit, effectlist, |
	rrefreshloop := runit0-1
	Loop %rrefreshloop%
		{
		rrefreshunit := runit%a_index%
		%rrefreshunit%_timer -= 100
		If (%rrefreshunit%_timer =< 0)
			{
			AA_Refresh(rrefreshunit)
			StringReplace, effectlist, effectlist, %rrefreshunit%|,,All
			%rrefreshunit%_timer = 0
			}
		}
	}
return
...
AA_Move(Direction) ; See below for potential directions.
{
	...
	%currentunit%_cooldown += (6 - movecd)
	AA_Scan()
	AA_ReverseScan(currentunit)
	AA_Refresh(currentunit)
	return
}

AA_Scan() ;checks to see if any unit is currently scanning it and if so, sets this unit to visible.
		;functions like a "reverse" scan.
{
global
...
Loop %scanloop%
	{
	scanner := currentunit
	If (%scanner%_scan = 0) ;check if scanning unit has scan function module
		return 0
	StringGetPos, Position, scanner, _
	StringLeft, scanningplayer, scanner, Position
	...
	Loop %scanloop%
		...
		If (player = scanningplayer)
			Continue
		xscandistance := abs(%scanunit%_posx - %scanner%_posx)
		yscandistance := abs(%scanunit%_posy - %scanner%_posy)
		scanrange := %scanner%_scan*20
		If (xscandistance <= scanrange) AND (yscandistance <= scanrange)
			%scanunit%_visiblecheck = 1
		}
	Loop 15 ;scan minerals routine
		...
		If mineral%a_index%_value >= 1 ;if a mineral has value > 1
			...
			xscandistance := abs(%target%_posx - %scanner%_posx)
			yscandistance := abs(%Target%_posy - %scanner%_posy)
			If (xscandistance <= scanrange) AND (yscandistance <= scanrange)
				%target%_visiblecheck = 1
			}
		}
	}
Loop %scanloop% ;now if any unit was in range, then the unit gets visible set to 1.
				;otherwise if no other unit is scanning the unit, the unit reverts to visible=0
	{
	scanunit := unit%a_index%
	StringGetPos, Position, scanUnit, _
	StringLeft, player, scanUnit, Position
	If (player = scanningplayer)
		Continue
	If (%scanunit%_visiblecheck = 1)
		%scanunit%_visible = 1
	Else If (%scanunit%_visiblecheck = 0)
		%scanunit%_visible = 0
	%scanunit%_visiblecheck = 0
	}
Loop 15
	{
	If mineral%a_index%_value >= 1 ;if a mineral has value > 1
		{
		scanunit = mineral%a_index%
		If (%scanunit%_visiblecheck = 1)
			%scanunit%_visible = 1
		Else If (%scanunit%_visiblecheck = 0)
			%scanunit%_visible = 0
...
AA_ReverseScan(ReverseScanUnit)
{
global
StringGetPos, Position, ReverseScanUnit, _
StringLeft, ReverseScanPlayer, ReverseScanUnit, Position
StringSplit, unit, AllUnitlist, |
refreshloop := unit0-1
Loop %refreshloop%
	{
	...
	StringGetPos, Position, scanUnit, _
	StringLeft, player, scanUnit, Position
	If (player = ReverseScanPlayer)
		Continue
	xscandistance := abs(%scanunit%_posx - %ReverseScanUnit%_posx)
	yscandistance := abs(%scanunit%_posy - %ReverseScanUnit%_posy)
	scanrange := %scanunit%_scan*20
	;msgbox, reverse scan in progress. Checking %ReverseScanUnit% against %scanunit% who has a scanrange of %scanrange%. if %xscandistance% <= %scanrange% AND %yscandistance% <= %scanrange% changing to visible.
	If (xscandistance <= scanrange) AND (yscandistance <= scanrange)
		{
		;msgbox, changing %ReverseScanUnit% to visible.
		%ReverseScanUnit%_visible = 1
		return
		}
	}
%ReverseScanUnit%_visible = 0
}
...
If unit
	{
	clipregionx := %unit%_posx-60
	clipregiony := %unit%_posy-60
	Gdip_SetClipRect(G, clipregionx, clipregiony, 120, 120)
	}
...
If (xattackdistance <= attackrange) AND (yattackdistance <= attackrange) ;check if target is in range
	...
	effectlist .= currentunit . "|"
	%currentunit%_timer := speedslider*2.5
	return
	}
Else
...
Loop 15
	...
	If mineral%a_index%_value >= 1 ;if a mineral has value > 1
		...
		If (xgatherdistance <= 20) AND (ygatherdistance <= 20) ;if gathering unit is within range, gather
			{
			;msgbox, found mineral within range. mineral = %target%
			If (%target%_visible = 0)
				{
				;msgbox target not visible.
				return 4
				}
			;msgbox, mineral within range and visible. commencing gather.
			%target%_value -= 1
			If (%target%_value = 0)
				mineralcurrent -= 1
