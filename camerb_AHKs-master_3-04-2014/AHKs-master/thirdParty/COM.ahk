...
COM_Error(hr = "", lr = "", pei = "", name = "")
{
	...
	errord("silent", "COM Error Notification", "Function Name:`t""" . name . """`nERROR:`t" . sError . "`t(" . nError . ")" . (bExcep ? SubStr(NumGet(pei+24) ? DllCall(NumGet(pei+24),"Uint",pei) : "",1,0) . "`nPROG:`t" . COM_Ansi4Unicode(NumGet(pei+4)) . COM_SysFreeString(NumGet(pei+4)) . "`nDESC:`t" . COM_Ansi4Unicode(NumGet(pei+8)) . COM_SysFreeString(NumGet(pei+8)) . "`nHELP:`t" . COM_Ansi4Unicode(NumGet(pei+12)) . COM_SysFreeString(NumGet(pei+12)) . "," . NumGet(pei+16) : "") . (lr ? "`n`nERROR2:`t" . sError2 . "`t(" . nError2 . ")" : ""))
	;IfMsgBox, No, Exit
        Exit
}
...
COM_ConnectObject(psource, prefix = "", DIID = "")
{
	...
	{
		errord("silent", A_ThisFunc, A_ScriptFullPath, "No Event Interface Exists!")
		Return
	}
