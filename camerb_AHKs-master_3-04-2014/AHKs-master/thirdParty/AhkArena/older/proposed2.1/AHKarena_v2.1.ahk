...
if NOT IsTempRunFile
	{
        TempAhkFileName=%A_Temp%\AhkArena.ahk
	...
	Else
        ...
        Loop, read, %ChoicePlayer1%
        {
                if RegExMatch(A_LoopReadLine, "(Script\(\)\{|Class\d:)")
                        ...
                        FileAppend, `nGosub`, %NamePlayer1%Class`%1currentclass`%, %TempAhkFileName%
                else
                        FileAppend, `n%A_LoopReadLine%, %TempAhkFileName%
        }
...
Loop %gamecount%
	...
	Loop
		...
		IF (gofirst = 1)
			player2script() ; execute player 1's script
		Else IF (gofirst = 2)
			player1script()
		Sleep %SpeedSlider%
		AA_Refresh() ; deletes animations, redraws board and all units.
...
Loop %1refreshloop%
	...
	Gosub, %player1%Class%1currentclass%
	}
return
}
...
Loop %2refreshloop%
	...
	Gosub, LagosClass%1currentclass%
	}
return
}
