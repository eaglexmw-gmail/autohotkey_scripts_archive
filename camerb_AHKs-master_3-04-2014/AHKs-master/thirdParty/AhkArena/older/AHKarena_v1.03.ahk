; version 1.031 Dynamic Include
#NoEnv
#SINGLEINSTANCE force
#include C:\Documents and Settings\Nathan\Desktop\Programing\AutoHotkey\GDI+\Gdip.ahk
#include player1script.ahk
#include player2script.ahk
...
if 0 < 2
{
  ...
  Gui, 3:Show,, Select Player1's Script
  WinWaitClose, Select Player1's Script ahk_class AutoHotkeyGUI
  Gui, 4:Add, DropDownList, vChoicePlayer2, %FileList%
  Gui, 4:Add, Button, Default, OK
  Gui, 4:Show,, Select Player2's Script
  WinWaitClose, Select Player2's Script ahk_class AutoHotkeyGUI
  If ChoicePlayer1 = %ChoicePlayer2%
  {
    MsgBox,48, Error, Player1's and Player2's scripts can not be the same!
    ExitApp
  }
...
Loop %gamecount%
	...
	Loop
		...
		%player1%script() ; execute player 1's script
		Sleep %SpeedSlider%
		AA_Refresh() ; deletes animations, redraws board and all units.
		%player2%script() ; execute player 2's script
		Sleep %SpeedSlider%
		AA_Refresh() ; deletes animations, redraws board and all units.
...
If (stats1+stats2+stats3+stats4+stats5+stats6 > 4)
	return
...
Loop 15 ;draw minerals
	...
	If (minsize = 0)
		continue
	Gdip_FillEllipse(G,pBGREEN55, mindrawx, mindrawy, size, size)
	munitvis := mineral%a_index%visible
	if (munitvis = 0)
		{
		mvisx := mineral%a_index%x+3
		mvisy := mineral%a_index%y+3
		Gdip_SetSmoothingMode(G, 1) 
		Gdip_FillRectangle(G,PbBLACK,mvisx,mvisy,4,4)
		}
	}
...
If (xattackdistance <= attackrange) AND (yattackdistance <= attackrange) ;check if target is in range
	...
	If (%target%_hp < 1) ;If target destroyed
		...
		;deathstar1x := attackanimationx2
		;deathstar1x := attackanimationy2-15
		;Gdip_DrawLine(G, pPYELLOWORANGE, attackanimationx2,attackanimationy2,attackanimationx2,attackanimationy2)
		FileRead, AllUnits, AllUnitList.txt
		StringReplace, AllUnits, AllUnits, %target%|,,All
		FileDelete, AllUnitList.txt
		FileAppend, %AllUnits%, AllUnitList.txt
		%deadplayer%unitcount -= 1
		classfind := Position+1
...
size := minval*4
Gdip_FillEllipse(G,pBGREEN55, randomx, randomy, size, size)
mvisx := mineral%a_index%x+3
mvisy := mineral%a_index%y+3
Gdip_FillRectangle(G,PbBLACK,mvisx,mvisy,4,4)
BitBlt(hdc_WINDOW,0, 0, 502,502, hdc_main,0,0) ;position of the GDI Image in the GUI
