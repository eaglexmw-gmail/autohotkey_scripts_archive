...
;this should only happen on the VM for right now
...
;this should only happen on the VM for right now
FireflyCheckin2(whoIsCheckingIn, Status)
{
   iniFolder:=GetPath("FireflyCheckinIniFolder")
...
GetNumberOfFeesNotYetAdded()
{
   ...
   FireflyBotEndTimer(timerFcn, A_ThisFunc, 0, 30)
...
ViewableIniFolder(iniFolder, viewableIniDestination)
{
   ...
   if NOT FileDirExist(iniFolder)
      fatalErrord("", "you did not specify an iniFolder", iniFolder)
