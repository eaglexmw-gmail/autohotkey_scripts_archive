...
; Extended 9/13/09 by kli6891
;   * added ability to "expose" minimized windows as well, and correctly minimize them again after expose is finished
...
; custom variables
miniList =         ; list of minimized windows
miniCount = 0         ; count of minimzed windows
...
draw_active:
  ...
  ; if ( pos > 0 )
      tooltip, "Title"  %title%  + %old_pos2% + %pos% + %winid%
...
fill_mini_List:
   ...
   Loop %listOfWindows%
   {
      ...
      if (title != "start" && title != "")
      {
         ...
         {
            miniCount++
            miniList%miniCount% := currentWindowID
   ...
   ;{
   ;   
   ;   currentWindowID := miniList%A_Index%
   ;
   ;   WinGetTitle, title, ahk_id %currentWindowID%
   ;
   ;   if (title != "start" && title != "")
...
restore_position:
   ; first, get the ID of the current active window
   WinGetActiveTitle, activeTitle
   WinGet, activeID, ID, %activeTitle%
   ;msgbox activeID = %activeID%`nminiCount = %miniCount%
   Loop, %miniCount%
   {
      currentWindowID := miniList%A_Index%
      ;msgbox activeID = %activeID%`ncurrentWindowID = %currentWindowID%
      ; minimizes all window as before, if they aren't the same
      if (currentWindowID != activeID)
      {
