; http://www.autohotkey.com/forum/topic40379.html
; =======================================================================================!§
; ===================================================================================
;
; To See and GO! , the "BROTHER" of this Folder!
; In EXPLORER press hotkey and show the Menu of Brother
; BROTHER> folders near this folder in the parent (?you know what i think?)
;
; Folder-Brothers.ahk
;  Jeudi 29 Janvier 2009
;
;       by soggos   > Since AHK All is Different
;                   > Great Thank's for the Forum, and the AHK Community !
;         
; ------------------------------------------------------------------------------
; Hotkeys
; ------------------------------------------------------------------------------
   ...
   #IfWinActive, ahk_group ExplorerGroup
      ...
      #<::goto Show_Menu_BRothers         ; Define your Hotkey to see the menu Brother.
...
; =======================================================================================!§
 ...
 ;hWnd := WinExist("A")         ;   for active window
                        ;   or for the window under the mouse:
                        ;      but you must comment #IfWinActive
 
   MouseGetPos,mx,my,mwin,mctrl   ; sinon prendre la fenetre sous le curseur
   WinGet, ws_ID, ID, ahk_id %mwin%   
   winActivate,ahk_id %mwin%
   direction = % GetWindowsExplorerPath(mwin)         ;%

   SplitPath, direction, OutFileName, OutDirActuel, ext,
   
   Gosub , BRothers
   
MI_ShowMenu("Menu_Frere")   ;   menu with icones         ;  gracias lexikos )
                     ;   menu , Menu_Frere, show      ; (normal menu without icones)
   Return
...
; =======================================================================================
   ...
   menu , Menu_Frere, Add         ;
   menu , Menu_Frere, DeleteAll   ;

combien:= 0                                             ; for define the number place in the menu
FileList =                                             ; List of Folder brother
Loop, %OutDirActuel%\*.*, 2                                 ; loop au-dessu just folder
    FileList = %FileList%%A_LoopFileName%`t%A_LoopFileFullPath%`n   ; simple Name for see in the Menu and the Full name for going to

Sort, FileList  ; Sort
                  ;   MsgBox , 4160 , , %FileList%
 ...
 Loop, parse, FileList, `n
 {
    ...
    StringSplit, FileItem, A_LoopField, %A_Tab%     ; Split into two parts at the tab char.> simple name and fullpath
   
   if FileItem1= Recycled                      ; don't want to see "Recycled" if foder=root
      continue
   
Menu, Menu_Frere, Add, %FileItem1%, Go_to_this_BROTHer

combien:= combien + 1    

   ifExist %FileItem2%\Desktop.ini                                 ; if icone folder define in the Desktop.ini get him
   {
   IniRead , IconFile, %FileItem2%\Desktop.ini,.ShellClassInfo, IconFile, 0
   IniRead , IconIndex, %FileItem2%\Desktop.ini,.ShellClassInfo, IconIndex, 0
   IconIndex := IconIndex + 1
   
   MI_SetMenuItemIcon("Menu_Frere", combien, IconFile, IconIndex, 20)

      }      
   else
   MI_SetMenuItemIcon("Menu_Frere", combien, "shell32.dll", 4, 20)         ; else get icone standard Folder


if FileItem1 = %OutFileName%                  ; if i am in the folder check the menu
Menu, Menu_Frere, check, %FileItem1%
 }

 
combien:= combien + 2
   Menu, Menu_Frere, Add,
   Menu, Menu_Frere, Add, Remonter Au Dossier Parent`t     &A, remonter   ;   Go to parent!
      MI_SetMenuItemIcon("Menu_Frere", combien, "shell32.dll", 147, 20)
      
   combien:= combien + 2
   Menu, Menu_Frere, Add,
   Menu, Menu_Frere, Add, Précédant`t<--  &Q , pagearriere
      MI_SetMenuItemIcon("Menu_Frere", combien, "shell32.dll", 220, 20)   ; <- come back
   
   combien:= combien + 1
   Menu, Menu_Frere, Add, Suivant`t-->  &W, pageavant
      MI_SetMenuItemIcon("Menu_Frere", combien, "shell32.dll", 220, 20)   ; -> Go one
...
; =======================================================================================
   ...
   Go_to_this_BROTHer:
    ...
    ControlGetPos, f_Edit1Pos,,,, Edit1, ahk_id %mwin%
        ControlGetText, f_text, Edit1, ahk_id %mwin%
        ControlSetText, Edit1, %OutDirActuel%\%A_ThisMenuItem%, ahk_id %mwin%
        ControlSend, Edit1, {Enter}, ahk_id %mwin%
        Sleep, 100 
        ControlSetText, Edit1, %OutDirActuel%\%A_ThisMenuItem%, ahk_id %mwin%
      send {tab}      ;
      send {Home}      ;
...
; ==========================================
   ...
   remonter:
    ...
    ControlGetPos, f_Edit1Pos,,,, Edit1, ahk_id %mwin%
        ControlGetText, f_text, Edit1, ahk_id %mwin%
        ControlSetText, Edit1, %OutDirActuel%, ahk_id %mwin%
        ControlSend, Edit1, {Enter}, ahk_id %mwin%
...
; and the BEST:   Menu Icons v2.1         great THANK's            (icones sur les Menus)
; =======================================================================================!§
; ====================================================================================
;
;  Menu Icons v2.1
