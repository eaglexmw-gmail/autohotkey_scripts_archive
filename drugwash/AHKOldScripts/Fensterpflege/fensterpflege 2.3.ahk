...
; http://de.autohotkey.com/forum/viewtopic.php?p=29606
;***************************************************************************************************
;**************************** Initialisierung ******************************************************
;***************************************************************************************************
...
helptext =
  (LTRIM
  Informationen zu HALWEGS FENSTERPFLEGE 2.3
  ...
  F9 (innerhalb von 10s - nach Erlöschen eines Tooltipps)`t- Erneute Anzeige der letzten Pflegemeldung (mit F9 wieder weg)
...
ELSE                                                                            ; Andernfalls wird ... 
  {
  IFEXIST, %A_SCRIPTDIR%\Images\MeineIcons.dll                                  ; ... in einem Unterverzeichnis "Iamges" des Skriptes nach einer "MeineIcons.dll" gesucht ... 
    iconfile = %A_SCRIPTDIR%\Images\MeineIcons.dll                              ; ... und die Bitmaps in dieser Datei verwendet.                               
...
look_for_new_window:
  ...
  IF (current_window_id AND current_window_title)                               ; Weitere Betrachtung nur wenn es gültige Fenster-Information gibt (ID und Titel). 
    ...
    IF (current_window_id = last_window_id)                                     ; Wenn das Fenster nicht gewechselt wurde ...
      IF (current_window_title <> last_window_title)                            ; ... aber der Fenstetitel sich geändert hat ...
        {
        STRINGRIGHT, end_of_window_title, current_window_title,5
        ;Textbeep("end_of_window_title=""" . end_of_window_title . """")  ;### Nur zum Testen 
        IF end_of_window_title NOT IN %no_change_proof1%,%no_change_proof2%,%no_change_proof3%,%no_change_proof4%         ; V2.3: Es werden bestimmte Fenster genannt, bei denen ein Wechsel des Fenstertitels nicht zu einer Pflege führen sollen
  ...
  IF to_proof                                                                   ; Wenn wir festgestellt haben, dass eine Untersuchung vonnöten ist ...
    ...
    IF proofed_ids NOT CONTAINS %current_window_id%
  ...
  ; 4. Fenster pflegen
    ...
    Resume_window_watching()                                                    ; ... Timer zur Fensterbeobachtung wieder einschlatenvorrübergehend ausschalten ...
    }
...
#+SCROLLLOCK::
  Textbeep("`n***************Pflege manuell aktivieren  *************")
  ...
  Textbeep("***************Pflege manuell aktivieren starten *************`n")
  proofed_ids =
  last_window_title =
RETURN
...
GUI, ADD, LISTVIEW,GListviewlabel GRID X5 W1100 -MULTI R40 NOSORTHDR LV0x10,Nr|Kategorie|Fenstertitel|Fenstertext|ms|Tasten|X|Y|Breite|Höhe|  
; Listview Zeilen
...
LV_MODIFYCOL(2,100)			        ; Kategorie
LV_MODIFYCOL(3,230)			        ; Title
LV_MODIFYCOL(4,320)		         	; Text
LV_MODIFYCOL(5, 40)			        ; Sleep (ms)
LV_MODIFYCOL(6,200)		       	  ; Keys
LV_MODIFYCOL(7, 40)			        ; X
LV_MODIFYCOL(8, 40)			        ; Y
LV_MODIFYCOL(9, 40)		       	  ; Width
LV_MODIFYCOL(10,40)		        	; Hight
...
GUI, ADD, BUTTON,X+75 W130                , Ein&stellungen
...
Window_found(fnew_window_id)
  ...
  LOOP, %number_of_handlers%                                                    ; Über alle Pflegeanleitungen ...
    ...
    IF current_window_title CONTAINS % wtitle%A_INDEX%                          ; Wenn Fenstertitel in Pflegeliste steht, dann ...
      {
      title_found_time := A_TICKCOUNT - id_taken_time                           ; ... notieren, wieviel Zeit die Suche gekostet hat (wird bei mehreren Fenstern erneut überschrieben) ...
      title_found_text:="Gefunden: Fenstertitel """ . wtitle%A_INDEX% . """ (nach " . title_found_time . " ms)"  ; ... und die MEldung dazu notieren.
      IF wtxt%A_INDEX%                                                          ; Nun den Fenstertext überprüfen, falls zur Pflegeanleitung ein Text hinterlegt wurde.
        ...
        IF NOT fnew_text                                                        ; Es wurde zu diesem Fenster noch kein Text ermittelt ...
          ...
          IF current_window_title CONTAINS Microsoft Internet Explorer          ; ... da warten wir auf den Fertig-Status beim IE-Fenster ...
            {
            IF NOT wait_for_IE_ready_state(fnew_window_id)                      ; (Falls der Fertig Status nicht hergestellt wurde ...
              BREAK                                                             ; ... beenden wir die Fensteranalyse)
            }
          ELSE
            SLEEP, % A_TICKCOUNT - id_taken_time < min_textsearch_time ? min_textsearch_time-(A_TICKCOUNT - id_taken_time):0  ; ... bzw. lassen mindestens min_textsearch_time ms vergehen ...
        ...
        IF fnew_text CONTAINS % wtxt%A_INDEX%                                   ; Nachschauen, ob der Text in der Pflegeanleitung im aktuellen Fenstertext gefunden wird
          ...
          plain_textsearch_time := A_TICKCOUNT - id_taken_time - title_found_time ; Da notieren wir noch, wieviel Zeit die Textsuche gekostet hat ...
          text_found_text:="Gefunden: Fenstertext """ . wtxt%A_INDEX% . """ (nach weiteren " . plain_textsearch_time . " ms, " . number_of_textsearches . " Fenstertexte geprüft)" ; ... und bereiten eine entsprechende Meldung vor
          BREAK
          }
        }
...
wait_for_IE_ready_state(wait_id)
  ...
  IFWINACTIVE, AHK_ID %wait_id%
    {
    Textbeep("Warte auf Fertig-Status Explorer-Fenster ...","leise",5000)
...
handle_window(fhandle_nr, fwindow_id)
  ...
  IF (wsleepms%fhandle_nr%  AND  wsleepms%fhandle_nr% <> 9999)
    {
    remaining_sleeptime :=  wsleepms%fhandle_nr% - (A_TICKCOUNT - id_taken_time)
  ...
  ELSE                                                                            ; Fenster konnte aktiviert werden, jetzt pflegen!
    ...
    IF wx%fhandle_nr% OR wy%fhandle_nr% OR  ww%fhandle_nr% OR  wh%fhandle_nr%   ; Müssen wir überhaupt verschieben?
      ...
      IF    (monitors <= 1 
        AND ((wx%fhandle_nr% > A_SCREENWIDTH OR wy%fhandle_nr% > A_SCREENHEIGHT) OR  consider_monitor_count))
      ...
      ELSE
        ...
        ELSE
          {
          Textbeep(" - Fenster wird verschoben: X: " . wx%fhandle_nr% . ", Y: " . wy%fhandle_nr% . ", Breite: " . ww%fhandle_nr% . ", Höhe: " . wh%fhandle_nr%,Shiftsound)
...
; Sucher für IE8
        ...
        SEND, %send_parts2%
        SLEEP, 200
        SEND, {F6}+{TAB}                                                              ; neu für IE8, ist noch im Test
        ; SLEEP, 5000
