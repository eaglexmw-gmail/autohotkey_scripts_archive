...
; Platform				: Win98SE // WinME // Win2k // WinXP // WinVista
...
MsgBox, Line %A_LineNumber%`n`nSourceFile: %SourceFile%`nnIndex: %nIndex%`nIconFile: %IconFile%`nshell: %shell%
	IconFile := In_File . "_" . SubStr(A_GuiControl , -3 , 1) . ".ico"
	SplitPath , IconFile , IconFileName
MsgBox, Line %A_LineNumber%`n`nSystemRoot: %SystemRoot%`nSourceFile: %SourceFile%`nIconFile: %IconFile%`nshell: %shell%
	
	SplitPath , SourceFile , , SourceFileDir , SourceFileExt
	StringLower, %SourceFileExt%, %SourceFileExt%
MsgBox, SourceFileDir: %SourceFileDir%`nSourceFileExt: %SourceFileExt%`nIn_Dir: %In_Dir%
...
BLOCK "VarFileInfo"
{
	...
	FileCopy, %A_ScriptDir%\%RC_EXE%, %A_WorkingDir%, 1
	s_ERR := "Couldn't copy " . RC_EXE . " to " . A_WorkingDir
	if ErrorLevel
		_ERROR_EXIT(Error_Message . s_ERR)

	s_CMD := """" . A_WorkingDir . "\" . RC_EXE . """"
			. " /r "
			. s_RC_FILE . " >> " . RC_Log
...
; --------------------------------------------------------------------------------
	...
	FileCopy, %A_ScriptDir%\%RES_Exe%, %A_WorkingDir%, 1
	s_ERR := "Couldn't copy " . RES_Exe . " to " . A_WorkingDir
	if ErrorLevel
		_ERROR_EXIT(Error_Message . s_ERR)

	s_CMD := """" . A_WorkingDir . "\" RES_Exe . """"
			. " -script "
			. """" . s_SC_FILE . """"
...
_Compile_AHK()
{
	...
	if A_OSType in WIN32_WINDOWS
		...
		FileCopy, %In_File%, %A_WorkingDir%\tmp.ahk, 1
;		FileCopy, %s_ICON%, %A_WorkingDir%\tmp.ico, 1
		s_IN := " /in tmp.ahk "
		s_OUT := " /out tmp.exe"
;		s_ICON := " /icon tmp.ico"
		s_ICON =
	...
	If Auto_Compile <> 2
	{
		...
		Msgbox , 262208 , %MSG_TITLE% , %s_LOG% , 7
	}
	if A_OSType in WIN32_WINDOWS
		FileCopy, tmp.exe, %Exe_File%, 1
	Return True
}
; --------------------------------------------------------------------------------
; Run after
...
Create_Charset_List()
{
	...
	If CCode_1 != 0
	{
		...
		CName_1 = 7-bit ASCII
		CCode_2 = 932
		CName_2 = Japan (Shift ñ JIS X-0208)
		CCode_3 = 949
		CName_3 = Korea (Shift ñ KSC 5601)
		CCode_4 = 950
		CName_4 = Taiwan (Big5)
		CCode_5 = 1200
		CName_5 = Unicode
