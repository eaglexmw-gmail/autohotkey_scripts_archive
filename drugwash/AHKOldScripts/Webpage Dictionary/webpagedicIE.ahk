...
;    return
  ...
  Else
  {
    WinGetClass, class, A
;   MsgBox, class: %class%
;    if (class <> "OpWindow" and class <> "MozillaUIWindowClass" and class <> "Internet Explorer_Server7")
    if (class <> "OpWindow" and class <> "MozillaUIWindowClass" and class <> "SlimBrowser MainFrame" and class <> "Internet Explorer_Server7")
     return
   
    WinGetTitle, title, A
   
    pos := RegExMatch(title, ".*\[(.+)\].*", match)
;MsgBox, Title: %title%`npos: %pos%   
    if (pos == 0)
MsgBox, we need to fix first RegEx
;      return
    ...
    if (pos > 0)
MsgBox, we need to fix second RegEx
 ;     return
