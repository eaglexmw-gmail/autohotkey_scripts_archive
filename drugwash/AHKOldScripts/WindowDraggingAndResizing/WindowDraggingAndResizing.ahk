         ...
         Loop          ;. Loop checks new mouse position and repositions (and resizes) a window
         {
            ...
            else
            {
               if WD_IsOnEdge ;. it just has left the edge
               {
                  ...
                  WinMove,ahk_id %GuiOutline%,,-5000,-5000, ;move Hidden window offScreen, so when it is shown again on a different edge without releasing the LButton the window outline does not show for a split second on the previous edge it was shown on.
