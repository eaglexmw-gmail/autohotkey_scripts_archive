; http://www.autohotkey.com/forum/viewtopic.php?p=268714#268714
/*
...
Loop 8
   ...
   ,"This ToolTip will be destroyed in " . 9-A_Index . " Seconds","o1 I2 B0000FF FFFFFFF D1")
...
ToolTip(10,"This ToolTip belongs to no control.`nIt is shown when you point mouse onto window area.`nIts option T (timer) is set to a high value to keep it displaying!"
      ,"Gui assosiated ToolTip","B0000FF FFFFFFF D99999999999 I1 P" . GuiHwnd)
...
Button1:
Loop 2
   ToolTip(1,"New Text " A_Index,"New Title " A_Index,"B0000FF FFFFFFF T200 Abutton" . A_Index . " P" . GuiHwnd)
REturn
...
ToolTip(ID="", text=" ", title="",options=""){ ;i x y b f d t o c m p a
   ...
   If P
   {
      If A {
         ...
         If !TOOLINFO_%ID%
         {
            VarSetCapacity(TOOLINFO_%ID%, 40, 0)
            TOOLINFO_%ID%:=Chr(40)
         } else
      ...
      If !TOOLINFO_%ID%
      {
         ...
         DllCall("SendMessage", "Uint", hWnd, "Uint", 0x405, "Uint", 0, "Uint", &TOOLINFO_%ID%)   ; TTM_DELTOOL
      If A
         Numput(A,TOOLINFO_%ID%,12)
      else
         Numput(P,TOOLINFO_%ID%,12)
   ...
   TTM_SETTITLE:
      ...
      ;0: None, 1:Info, 2: Warning, 3: Error. n > 3: assumed to be an hIcon.
