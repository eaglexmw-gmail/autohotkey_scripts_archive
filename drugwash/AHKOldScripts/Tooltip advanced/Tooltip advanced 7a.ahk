...
ToolTip(ID="", text=" ", title="",options=""){
   ...
   local option, a, b, c, d, f, i, m, o, p, l, v, update
   C_ID:=ID
   ...
   If (!hwnd and text)
   {
      ...
      If (title){
         ...
         DllCall("DestroyWindow","Uint",hwnd), hWndArray%ID%:="",TOOLINFO_%ID%:=""
      DetectHiddenWindows,%#_DetectHiddenWindows%
      Return
   }
   ...
   If (D and !P)
   {
      ...
      DllCall("DestroyWindow","Uint",hwnd), hWndArray%C_ID%:=0,TOOLINFO_%ID%:=""
   }
   If !P
   {
      If (!(x . y) and update) {
