...
if swabt
	...
	sw := "Show"
	Gui, 2:Font
	GuiControl,Font, abted
	Gui, 2:Color, Default, Default
	}
swabt := not swabt
GuiControl, 2:%sw1%, abt1
GuiControl,, abted, %aboutcopy%
GuiControl, 2:%sw%, abted
return
...
abt_TT := "About EmoPac"
Gui, Font, CRed bold
Gui, Add, Text, x+3 y5 w35 h14 Hidden vexpLab AltSubmit, Expert
Gui, Add, Text, x+2 yp w38 h14 Hidden vdebLab AltSubmit, Debug
Gui, Font
...
Gui, Add, Checkbox, xp yp+43 wp h16 vmakeF grefresh AltSubmit, w/ folders
makeF_TT := "When enabled, each protocol will have its images in an omonimous separate folder"
Gui, Add, Checkbox, xp yp+21 wp h16 ve10 grefresh Hidden AltSubmit, create emo
e10_TT := "Creates .emo files (only useful for Emoticons plug-in's developer)"
Gui, Add, Button, xp yp+18 w74 h20 vtt3 AltSubmit BackgroundTrans, &Clear
...
Gui, Add, GroupBox, x322 y200 w87 h252, Actions
...
Gui, Add, GroupBox, x7 y189 w586 h354 AltSubmit vegb, Emoticon images
...
statbar_TT :=
SB_SetParts(80, 70, 310, 75, 45)
Gui, Add, Progress, X15 Y545 W300 H11 -Smooth vloader
loader_TT :=
GuiControl, Hide, loader
Gui, Add, Text, x414 y543 w165 h16 Hidden ve8 c%color% Right AltSubmit gopenIni, %inipath%
e8_TT := "Click to edit emopac.ini"
Gui, Add, Text, x15 y543 w295 h16 Hidden ve9 c%color% AltSubmit gopenPath, %fdpath%
...
Gui, Show, x420 y85 w600 h504 AltSubmit, EmoPaC
;Gui, +Resize -MaximizeBox +MinSize600x504 +MaxSize600x730
Gui, +Resize -MaximizeBox +MinSize600x504 +MaxSize%fullW%x%fullH%
...
;------- RESIZE -------
GuiSize:
GuiControl, Move, e8, % "x" A_GuiWidth - 180 "y" A_GuiHeight - 37
GuiControl, Move, e9, % "y" A_GuiHeight - 37
GuiControl, Move, loader, % "y" A_GuiHeight - 35
GuiControl, -Redraw, srcL
GuiControl, -Redraw, destL0
Loop % countP
	GuiControl, -Redraw, destL%A_Index%
egbh := A_GuiHeight - 229
egbw := A_GuiWidth - 14
GuiControl, Move, egb, % "h" egbh "w" egbw
srcLh := A_GuiHeight - 255
destLw := A_GuiWidth - 427
GuiControl, Move, srcL, % "h" srcLh
GuiControl, Move, destL0, % "h" srcLh "w" destLw
Loop % countP
	GuiControl, Move, destL%A_Index%, % "h" srcLh "w" destLw
GuiControl, +Redraw, srcL
GuiControl, +Redraw, destL0
Loop % countP
	GuiControl, +Redraw, destL%A_Index%
...
else if (sT = "ico" OR sT = "jpg" OR sT = "jpeg" OR sT = "png" OR sT = "tiff" OR sT = "swf")
	mT := 0x000000
SetFormat, integer, D
return
...
ButtonAddimage:
Gui, ListView, srcL
FileSelectFile, fpath, M27,, Select image files to add to the pack, Images (*.ico; *.bmp; *.gif; *.jpg; *.png; *.tiff; *.swf) 
/*
	{
...
if debug
	...
	MsgBox, 0, Debug, Image #%selItemID% has been assigned to label %dselItemID% in protocol %last%`nlabel%last%x%dselItemID% = %fullP%`n%temp%
	}
return
;---------------------------- END ASSIGN FILE --------------------------
...
ButtonBlank<<:
Gui, ListView, destL%wP%
if ( not dselItemID or label%last%x%dselItemID% := )
	return
if not exp
	{
	MsgBox, 4, Warning, Are you sure you want to unassign this image?
		IfMsgBox No
			return
	}
label%last%x%dselItemID% :=
assigned%last% -= 1
LV_Modify(dselItemID, "Icon1")
Gui, Submit, NoHide
return
;-------------------------- END UNASSIGN FILE ------------------------
...
IL_Destroy(srcIDL)
Loop % countP
	TV_Modify(Pr%A_Index%, "-Check -Select -Bold")
Control, Enable,, SysTreeView321
TV_Modify(Pr1, "Check Select Bold VisFirst")
return
;--------------------------- END CLEAR ALL DATA ------------------
...
Loop, Read, %loadmep%
	...
	if (index < 4)
		{
		StringLeft, temp1, currLine, 5
		StringLeft, temp2, currLine, 8
		StringLeft, temp3, currLine, 12
		if (index = 1 and temp1 = "Name:")
			{
			StringTrimLeft, pname1, A_LoopReadLine, 6
			continue
			}
		if (index = 2 and temp2 = "Creator:")
			{
			StringTrimLeft, pauthor1, A_LoopReadLine, 9
			continue
			}
		if (index = 3 and temp3 = "Updater URL:")
			{
	...
	if not currLine
		continue
	StringLeft, temp4, currLine, 1
	if debug
		MsgBox, currLine=%currLine%`ntemp4=%temp4%
	if (temp4 = "#")
		{
		...
		if (currLine = "Global")
			...
			Loop, %countP%
				{
				Prt := Pr%A_Index%
				TV_GetText(temp5, Prt)
				if debug
					MsgBox, temp5=%temp5%. Prt=%Prt%	
				if (currLine = temp5)
					{
					currList = destL%A_Index%
					wP := A_Index
					break
					}
				if debug
					MsgBox, We loop to find the right protocol.`nPrt=%Prt%
				}
			}
		gosub tvselect
		last := wP
	...
	if debug
		MsgBox, last=%last%`ncurrLine=%currLine%`ncurrList=%currList%`nWe are reading line #%index%
	StringReplace, currLineN, currLine, %A_Space%=%A_Space%, ¶, All
	Loop, Parse, currLineN, ¶, "
	...
	if not tmp
		...
		{
		StringTrimLeft, label, a1, tmp
		checkF := 1
		}
		if checkF and not makeF
			GuiControl,, makeF, 1
	lastL := countL%wP%
	Loop, %lastL%
		{
		temp7 := A_Index
		Gui, ListView, %currList%
		LV_GetText(temp6, temp7, 2)
		if debug
		MsgBox, We are at item %temp7% (%temp6%) in %currList%, looking for %label%. Still looping...
		if (temp6 = label)
			{
		if debug
		MsgBox, Protocol %wP% has %lastL% labels.`nWe are at label %temp7% (%label%) in list %currList%, assumed to be identical with %temp6%.
			fpath := loadpath . "`n" . a2
			fullP := loadpath . "\" . a2
			dselItemID := temp7
			Gui, ListView, srcL
			...
			gosub assignone
			if debug
				MsgBox, fpath=%fpath%`nfullP=%fullP%`nselItemID=%selItemID%`ndselItemID=%dselItemID%`ntemp6=%temp6%
			break
			}
		}
	sleep, 300
...
GuiControl, Show, srcL
gosub tvselect
last := 1
Gui, ListView, destL1
GuiControl, Show, destL1
if (index < 5)
	MsgBox, This pack is a simple one. Reading such pack is not possible yet.
return

tvselect:
TV_Modify(Pr%last%, "-Check -Select -Bold")
TV_Modify(Pr%wP%, "Check Select Bold VisFirst")
Gui, Submit, NoHide
return
;------------------------- END LOAD & PARSE PACK --------------------
...
Loop, %countP%
	...
	fold0 := filepath . "\" . pro
	if (InStr(FileExist(fold0), "D") and (not assigned%writeP% or not makeF))
		FileRemoveDir, %fold0%, 1
