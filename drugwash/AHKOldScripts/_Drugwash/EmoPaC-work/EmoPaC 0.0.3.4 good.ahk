...
;------------------------ COPYRIGHT STUFF ----------------------------
about:
Gui, Font, S7 CRed norm, Verdana
Gui, 2:Add, Edit, w380 h405 Multi ReadOnly Center +Owner1 vabt1,
(
EmoPaC - Emoticon Pack Creator`n
(partly) Generated by SmartGUI Creator
Created by Drugwash drugwash@gmail.com
April 2008
version 0.0.3.4 alpha
••••••••••••••••••••••••••••••••••••••••••••••••••••••••••
...
abt1_TT :=
Gui, 2:Add, Button, x160 y415 w80 h25 gabtC, Close
Gui, 2:Show, w400 h450, About EmoPac
Gui, Font, norm
return
...
Gui, Add, Text, x57 y0 w286 h17 AltSubmit BackgroundTrans, Miranda IM Emoticon Pack Creator
Gui, Font, S8
Gui, Add, Text, x94 y18 w215 h13 AltSubmit BackgroundTrans, for Pescuma's Emoticons plug-in
Gui, Font, S6 norm, MS sans serif
Gui, Add, Button, 0x8000 x490 y1 w100 h33 Right AltSubmit BackgroundTrans vabt gabout, by Drugwash `n©2008%A_Space%
abt_TT := "About EmoPac"
Gui, Add, Pic, x456 y1 Icon1, EmoPaC.exe
;Gui, Add, Pic, x456 y1 w32 h32 AltSubmit BackgroundTrans, data/EmoPaC.ico
;Gui, Add, Text, x490 y12 w102 h15, by Drugwash ©2008
Gui, Add, GroupBox, x7 y32 w310 h80 AltSubmit BackgroundTrans, Pack data
Gui, Add, Text, xp+9 yp+15 w80 h16 Right AltSubmit BackgroundTrans, Name :
Gui, Add, Text, xp yp+21 wp hp Right AltSubmit BackgroundTrans, Author :
Gui, Add, Text, xp yp+21 wp hp Right AltSubmit BackgroundTrans, Updater URL :
Gui, Add, Edit, x98 y46 w210 hp+1 vpname gvalid AltSubmit BackgroundTrans
Gui, Add, Edit, xp yp+21 wp hp vpauthor gvalid AltSubmit BackgroundTrans
Gui, Add, Edit, xp yp+21 wp hp vpupdater gvalid AltSubmit BackgroundTrans
Gui, Add, GroupBox, x322 y32 w87 h166 AltSubmit BackgroundTrans, Main
...
Gui, Add, Checkbox, xp+8 yp+41 w58 h32 vmakeF grefresh AltSubmit BackgroundTrans, Create folders
...
Gui, Add, GroupBox, x7 y113 w310 h84, Codes
Gui, Add, Text, x16 y128 w80 h16 Right AltSubmit BackgroundTrans, Standard :
Gui, Add, Edit, x98 y127 w210 h17 ReadOnly vcodes AltSubmit BackgroundTrans
Gui, Add, Text, x16 y149 w80 h16 Right Hidden ve1 AltSubmit BackgroundTrans, Custom codes :
e1_TT := "Developer option - allows editing for the current emoticon's codes sequence"
Gui, Add, Edit, x98 y148 w210 h17 Hidden ve2 gvalid AltSubmit BackgroundTrans
Gui, Add, Button, x98 y169 w67 h20 Hidden ve3, &Default
e3_TT := "Revert current emoticon's code sequence to the default one"
Gui, Add, Button, x170 y169 w67 h20 Hidden ve4, &Undo
e4_TT := "Undo last operation on codes sequence"
Gui, Add, Button, x242 y169 w67 h20 Hidden ve5, &Merge
e5_TT := "Merge the currently added codes with the already existant sequence"
Gui, Add, GroupBox, x322 y210 w87 h337, Actions
Gui, Add, Button, xp+7 y225 w74 h39 Default vtt4, &Folder
tt4_TT := "Open a folder containing emoticon images to be added to the pack"
Gui, Add, Button, xp y265 wp h20 vtt5, Add &image
tt5_TT := "Add a single image to the list of usable emoticon images"
Gui, Add, Button, xp y286 wp hp vtt6, &Rem. image
tt6_TT := "Remove an image from the list of usable images (currently not implemented)"
Gui, Add, Button, xp y308 w36 hp Hidden ve6, « ±
e6_TT := "Toggle left-hand list between large icon view and report view"
Gui, Add, Button, x367 y308 w36 hp Hidden ve7, ± »
e7_TT := "Toggle right-hand listbetween large icon view and report view"
Gui, Add, Button, x329 y330 w74 h39 vtt7, &Assign >>
tt7_TT := "Assign the image in the left-hand panel to the selected emoticon label in the right-hand panel"
Gui, Add, Button, xp y370 w74 h19 vtt8, &Blank <<
tt8_TT := "Remove the image assigned to the selected label"
Gui Add, Button, xp y438 w36 h19 vtt9 gAniGifZ+, ++
tt9_TT := "Zoom in (only for animated GIFs)"
Gui Add, Button, x367 yp wp hp vtt10 gAniGifZ-, - -
tt10_TT := "Zoom out (only for animated GIFs)"
Gui, Add, GroupBox, x414 y32 w177 h166, Protocols
Gui, Add, Radio, x421 y152 w60 h16 AltSubmit BackgroundTrans gdoAll, all
Gui, Add, Radio, xp y173 wp hp Checked vselP AltSubmit BackgroundTrans gdoSel, select
if Unicode
	Gui, Add, TreeView, x482 y46 w104 h150 Checked Sort -HScroll -Lines 0x1000 AltSubmit vProto gdoProto AltSubmit BackgroundTrans
else
	Gui, Add, TreeView, x482 y46 w104 h150 Checked Sort -HScroll -Lines 0x1000 AltSubmit vProto gdoProto
gosub iniP
;-------- ANIGIF CODE HERE ------------
Gui +LastFound
guiID := WinExist()
Gui, Add, GroupBox, x329 y386 w74 h50
hAniGif := AniGif_CreateControl(guiID, 330, 395, 72, 38, "center")
If hAniGif is not integer
	MsgBox Error in %hAniGif%
AniGif_SetBkColor(hAniGif, 0xFFFFFF)
;-------- END ANIGIF CODE ------------
Gui, Add, GroupBox, x7 y199 w586 h356 AltSubmit BackgroundTrans, Emoticon images
if Unicode
	Gui, Add, ListView, x13 y216 w305 h330 -Multi NoSortHdr -ReadOnly AltSubmit BackgroundTrans Icon Grid vsrcL gsrcSel, Name|Size(bytes)|Last modified
else
	Gui, Add, ListView, x13 y216 w305 h330 -Multi NoSortHdr -ReadOnly AltSubmit Icon Grid vsrcL gsrcSel, Name|Size(bytes)|Last modified
Loop, % countP + 1
	{
	temp := A_Index - 1
	if Unicode
		Gui, Add, ListView, x414 y216 w173 h330 -Multi NoSortHdr ReadOnly AltSubmit BackgroundTrans Icon Sort Hidden vdestL%temp% gdestSel, Protocol|Common|Codes
	else
		Gui, Add, ListView, x414 y216 w173 h330 -Multi NoSortHdr ReadOnly AltSubmit Icon Sort Hidden vdestL%temp% gdestSel, Protocol|Common|Codes
	}
IconView := true
IconViewL := true
gosub iniD
...
Gui, Add, StatusBar, -0x800 vstatbar
statbar_TT :=
SB_SetParts(80, 70, 320, 70, 50)
Gui, Add, Progress, X15 Y560 W300 H11 vloader
GuiControl, Hide, loader
Gui, Add, Text, x414 y559 w165 h14 Hidden ve8 c%color% Right AltSubmit BackgroundTrans gopenIni, %inipath%
e8_TT := "Click to open the folder containing emopac.ini"
Gui, Add, Text, x15 y559 w295 h14 Hidden ve9 c%color% AltSubmit BackgroundTrans gopenPath, %fdpath%
e9_TT := "Click to open the folder containing the above emoticon images"
Gui, Add, Text, vdeb1 Hidden AltSubmit BackgroundTrans x16 y532 w300 h14 cRed, AGuiEvent: %A_GuiEvent%, AEventInfo: %A_EventInfo%
Gui, Add, Picture, 0x200 0x40 AltSubmit Hidden x347 y395 w38 h38 vprev, %debugpath%
Control, Disable,, SysListView321
if Unicode
	Gui, Add, Pic, 0x4000000 x0 y0 w600 h595 AltSubmit, data/back.png
Gui, Show, x420 y85 w600 h595  AltSubmit BackgroundTrans, EmoPaC
...
TV_Modify(Pr%last%, "Check Select VisFirst")
wP := last
...
Loop, 99	; assumingly there will never be more than 99 supported protocols :-)
	...
	if protoL = Default
		{
		Pr%A_Index% := TV_Add(protoL, 0, "Check")
		last := 1
		}
	else
...
;IL_Add(srcIDL, "data/noimg1.ico", 0xFFFFFF, 1)
...
if debug
	{
	Gui, Add, Text, x329 y503 w70 h14 cYellow AltSubmit BackgroundTrans, Index: %colInd%
	Gui, Add, Text, xp yp+14 wp hp cYellow AltSubmit BackgroundTrans, Addr: %indB%
	Gui, Add, Text, xp yp+14 wp hp cYellow AltSubmit BackgroundTrans, Mask: %mT%
	}
;		GuiControl, Hide, prev1
...
FileSelectFolder, filepath,, 3, Please select or create a folder to save the pack to:
if NOT filepath
	{
