...
xpath(ByRef doc, ByRef iter, step, set = "") {
...
if (t = "") {
  t = /
	...
	If (InStr(step, "|")) {
		...
		Loop, %s0%
			res .= xpath(doc, iter, s%A_Index%, set) . ","
		...
		If ax !=
		{
			...
			Loop, Parse, ax, ] ; for each predicate
			{
				...
				If (!RegExMatch(A_LoopField, "/?(.*?)\[(.+?)(?:\s*([<>!=]{1,2})\s*(['""])?(.+)(?(4)\4))?\s*$", a))
					Continue
				a1 = %a1%/
; HACK FOR select:/*/*[critera1][criteria2]
if (A_Index = 1)
	b1 := a1
else if (stsl = 1)
	a1 := b1
		...
		If (InStr(fn, "+count()")) ; increment counter if count() function is used
			ct++
		else If (InStr(fn, "+nodes()")) ; increment counter if count() function is used
			res = %res%%rem%,
	...
	{
		If (InStr(set, "node:") == 1) {
			set := SubStr(xpath(doc, iter, SubStr(set, 6) . "/rawsrc()"), 2)
...
xpath_save(ByRef doc, src = "") {
	...
	xml := RegExReplace(SubStr(xml, 1), "S)(?<=<)(\/)?(?:(\w+)\/)+(?(1)|:: )", "$1$2") ; remove metadata
...
;				xml = %xml%&lt;%A_LoopField% ; convert to escaped entity value
	...
	StringReplace, doc, doc, `r, , All ; normalize linefeeds:
	StringReplace, doc, doc, `n, , All
	doc := RegExReplace(doc, ">\s*<", "><") ; remove whitespace between tags
