...
;Localization for the "My Documents" folder (Vim displays this folder as "~\My Documents" - not a complete path, so we have to detect it and replace it with the actual path.)
...
IfWinNotExist, %TitleEnd% ahk_class Vim
{
...
WinGetTitle, WinTitle, %TitleEnd% ahk_class Vim 
WinGet, EWID, ID, %WinTitle% 
...
;~ 	IfEqual, Test, *, StringTrimRight, FileName, FileName, 1
	...
	ePos := InStr(WinTitle, ".ahk")
	If(ePos)	;if found
	{	
		StringLeft, File_Name, WinTitle, ePos+4
		RegExMatch(WinTitle, "\((.*?)\)", File_Path)
		StringReplace, File_Path1, File_Path1, %MyDocuments%, %A_MyDocuments%
		FileName := File_Path1 . "\" . File_Name
	}
	Else	;not found
	{	FileName=
	}
