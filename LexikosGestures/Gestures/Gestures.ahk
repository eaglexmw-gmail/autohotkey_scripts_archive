...
; TODO:
;
;   - Replace some subroutines with functions (localize some variables).
;   - Change m_Interval to be independent of how long an iteration takes.
;   - Revise variable names (remove m_ prefix..., consider hdc_canvas etc.).
;       - Consider renaming m_ScrolledWheel- see XButton1 hotkey in Gestures_User.ahk.
;   - Revise/clean up comments.
;   - Document the new structure and important differences.
;   - Automatically detect changes to script files.
;   - Add option for button-less gestures.
;   - Trails: make nodes prettier and more customizable (colour, ellipse/rect/pointer, etc.)
;   - Trails: add option for transparency level.
;   - Trails: optional fade in/out effects.
;   - Trails: add optional GDI+ support (for smoother lines).
;
;   - When run without any config, prompt user to configure?
;   - Config UI.
...
m_LowThreshold = 10     ; Minimum distance to register as a gesture "stroke."
...
;m_InitialZoneCount = 4 ; If set, the number of zones allowed for the *first* stroke.
m_DisableDing = 0
m_Delimiter = _
...
Menu, Tray, Add, Edit Gestures.&ini         , TrayMenu_Edit
...
; Set m_LastGestureKey to a sensible default in case it is somehow used before GestureKey_Down runs.
...
GestureKey_Down:
    ...
    if (%c_GesturePrefix%_WheelUp!="" || IsLabel(c_GesturePrefix "_WheelUp"))
        Hotkey, *WheelUp, GestureWheelUp, On
    if (%c_GesturePrefix%_WheelDown!="" || IsLabel(c_GesturePrefix "_WheelDown"))
    ...
    Loop
    {
        ...
        {
            ; Timed out.
            if (m_Gesture!="" && m_ActiveTimeoutMode=2)
            {
    ...
    if (m_Timeout && A_TickCount-beginTimeout > m_Timeout)
    {
        ...
        if !m_DisableDing
            SoundPlay, *64
        G_ExitGesture(m_DefaultOnTimeout)
        return
    }

    if m_Gesture !=
    {
        if !G_PerformAction(m_Gesture) && !m_DisableDing
            SoundPlay, *48
    }
    else
        G_ExitGesture(true)
...
{
    local action, params, final_name, list := c_GesturePrefix ",Default"
